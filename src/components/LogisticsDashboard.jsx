...
import { Truck, AlertTriangle, CheckCircle, Thermometer, MapPin, Search, Droplets } from 'lucide-react';
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h2 className={`text-2xl font-bold mb-4 ${darkMode ? 'text-white' : 'text-gray-800'}`}>Logistics Dashboard</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div className={`p-6 rounded-xl shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Temperature Alerts</p>
                  <p className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>2</p>
                </div>
            ...
            <div className={`p-6 rounded-xl shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                    {products.filter(p => p.status === 'At Store').length}
                  </p>
                </div>
            ...
            <div className={`p-6 rounded-xl shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Avg Temp</p>
                  <p className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                    {Math.round(products.reduce((acc, p) => acc + p.temperature, 0) / products.length * 10) / 10}°C
                  </p>
                </div>
          ...
          {filteredProducts.map((product) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className={`px-3 py-1 rounded-full text-sm ${product.status === 'In Transit' ? 'bg-blue-100 text-blue-800' : product.status === 'At Store' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                  {product.status}
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-800'}`}>{product.temperature}°C</span>
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Humidity:</span>
                  <div className="flex items-center">
                    <Droplets className="w-4 h-4 mr-1 text-blue-500" />
                    <span className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-800'}`}>{product.humidity}%</span>
                  </div>
                </div>
              </div>
              <div className="mt-4 flex space-x-2">
                ...
                <button className="flex-1 bg-yellow-600 text-white py-2 px-4 rounded-lg hover:bg-yellow-700 text-sm">
                  Report Issue
                </button>
              </div>
            </div>
